...
interface UserMenuProps {
  /** Fallback si jamais le fetch échoue */
  name?: string;
  email?: string;
  plan?: string;
}
...
/**
 * On ne renvoie plus que 2 valeurs possibles :
 * - "ADMIN"
 * - "PREMIUM"
 */
...
export function UserMenu({
  ...
  const badgeLabel = displayPlan; // ADMIN ou PREMIUM uniquement
  ...
  return (
    <>
        ...
        <AnimatePresence>
          {open && (
            ...
            >
              ...
              <button
                className="flex w-full items-center justify-between px-4 py-2.5 text-[11px] text-slate-200 hover:bg-slate-800/60 transition-colors"
                onClick={() => handleOpenBilling("billing")}
              >
                <span>Abonnement &amp; facturation</span>
                <span className="text-[10px] text-slate-400">Gérer</span>
              </button>
              ...
              <button
                ...
                className="flex w-full items-center justify-between px-4 py-2.5 text-[11px] text-red-400 hover:bg-red-500/10 transition-colors"
              >
      ...
      {/* PANEL CENTRAL ABONNEMENT / PARAMÈTRES */}
      <BillingPanel
        open={billingOpen}
        onClose={handleCloseBilling}
        name={displayName}
        email={displayEmail}
        planLabel={displayPlan} // ADMIN ou PREMIUM
        isAdmin={isAdmin}
